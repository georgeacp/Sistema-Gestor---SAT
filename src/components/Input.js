...
export default function Input({ placeholder, type, id, className, text }) {
  const [value, setValue] = useState(text);

  return (
    <div className={`bg-white rounded-lg group ${className}`}>
      <div className="relative bg-inherit">
        <input type={type} id={id} value={value} className="peer bg-transparent h-10 w-full rounded-lg text-gray-800 placeholder-transparent ring-2 px-2 ring-gray-500 group-focus:ring-sky-600 focus:outline-none group-focus:border-rose-600 text-sm" placeholder={placeholder} onChange={(event) => {
